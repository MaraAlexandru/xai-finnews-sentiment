# scripts/13_backtest_sector_strategies.py
"""
Backtests: sentiment-tilt strategies on sector ETFs.

Signal:
  - Daily mean sentiment per industry (EN, weak labels).
  - Rolling 63-day z-score z_t.
  - Position_t = 
      Long-Only  : 1 if z_t > +1.0, 0 if |z_t| <= 1.0, 0 if z_t < -1.0
      Long-Short : 1 if z_t > +1.0, -1 if z_t < -1.0, 0 otherwise
  - Trades occur at next close: use position_{t-1} * return_t

Costs:
  - Per change in position: cost_bps (default 5 bps) * |pos_t - pos_{t-1}|
  - Subtracted from daily return.

Outputs (per industry):
  outputs/backtest/<slug>/daily_<variant>.csv        (date, ret, pos, cost, eq_curve)
  outputs/backtest/<slug>/summary_<variant>.json     (metrics + settings)
Global:
  outputs/backtest/summary.csv

Run: python scripts/13_backtest_sector_strategies.py
"""
from pathlib import Path
import re, json
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

from _config import MARKETAUX_ARTICLES_CSV, OUTPUTS_DIR, SEED

# ---- Paths & constants ----
ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT / "data"
ETF_DIR = DATA_DIR / "market" / "etf"

OUT_DIR = OUTPUTS_DIR / "backtest"
SIG_CSV = OUTPUTS_DIR / "weaklabel_lr" / "daily_sentiment_by_industry.csv"

COST_BPS = 5        # per trade (change in position), in basis points
Z_THR = 1.0         # z-score threshold
MIN_OVERLAP = 25    # minimum overlapping days between sentiment & ETF to run

SECTOR_TO_ETF = {
    "Communication Services": "XLC",
    "Consumer Cyclical":      "XLY",
    "Energy":                 "XLE",
    "Financial Services":     "XLF",
    "Healthcare":             "XLV",
    "Industrials":            "XLI",
    "Real Estate":            "XLRE",
    "Utilities":              "XLU",
    "Technology":             "XLK",
}

def _slug(s): 
    return re.sub(r"[^a-z0-9]+","-", (s or "unknown").lower()).strip("-")

def _read_etf(ticker: str):
    """Robust CSV reader that tolerates a second header row like ',XLC,XLC,...'."""
    p = ETF_DIR / f"{ticker}.csv"
    if not p.exists():
        return None
    df = pd.read_csv(p)
    # Drop accidental second header row
    if df.shape[0] and (pd.isna(df.iloc[0, 0]) or str(df.iloc[0, 0]).strip() == ""):
        df = df.iloc[1:].reset_index(drop=True)
    # Ensure proper parsing
    if "Date" not in df.columns or "Adj Close" not in df.columns:
        df = pd.read_csv(p, parse_dates=["Date"])
    else:
        df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
    df = df.dropna(subset=["Date"]).sort_values("Date").reset_index(drop=True)
    df["date"] = df["Date"].dt.date
    df["Adj Close"] = pd.to_numeric(df["Adj Close"], errors="coerce")
    df["ret"] = df["Adj Close"].pct_change()
    return df[["date","ret"]].dropna()

def _try_vader(texts, pos_thr=0.05, neg_thr=-0.05):
    try:
        from nltk.sentiment import SentimentIntensityAnalyzer
        import nltk
        try: nltk.data.find("sentiment/vader_lexicon.zip")
        except LookupError: nltk.download("vader_lexicon")
        sia = SentimentIntensityAnalyzer()
        labs = []
        for t in texts:
            c = sia.polarity_scores(t or "")["compound"]
            labs.append(2 if c >= pos_thr else 0 if c <= neg_thr else 1)
        return labs, "VADER"
    except Exception:
        return None, None

def _lm_label(texts):
    pos_p = DATA_DIR / "lexicons" / "lm" / "lm_positive.txt"
    neg_p = DATA_DIR / "lexicons" / "lm" / "lm_negative.txt"
    if not (pos_p.exists() and neg_p.exists()):
        return None, None
    pos = set(w.strip().lower() for w in pos_p.read_text(encoding="utf-8").splitlines() if w.strip())
    neg = set(w.strip().lower() for w in neg_p.read_text(encoding="utf-8").splitlines() if w.strip())
    tok_re = re.compile(r"[A-Za-z]+")
    y = []
    for t in texts:
        toks = [x.lower() for x in tok_re.findall(t or "")]
        p = sum((z in pos) for z in toks); n = sum((z in neg) for z in toks)
        sc = p - n
        y.append(2 if sc >= 1 else 0 if sc <= -1 else 1)
    return y, "LM"

def _load_or_build_signal():
    if SIG_CSV.exists():
        return pd.read_csv(SIG_CSV, parse_dates=["date"]).assign(date=lambda d: d["date"].dt.date)
    # Build from Marketaux
    if not MARKETAUX_ARTICLES_CSV.exists():
        print(f"[ERROR] Missing: {MARKETAUX_ARTICLES_CSV}")
        return None
    df = pd.read_csv(MARKETAUX_ARTICLES_CSV, parse_dates=["published_at"])
    df["date"] = df["published_at"].dt.date
    df = df[df["language"].fillna("unk").str.lower().eq("en")].copy()
    df["text"] = df["text"].fillna("")
    y, src = _try_vader(df["text"].tolist())
    if y is None:
        y, src = _lm_label(df["text"].tolist())
    if y is None:
        print("[ERROR] Weak labels unavailable.")
        return None
    df["y"] = y
    df["s"] = df["y"].map({0:-1,1:0,2:+1})
    sig = (df.groupby(["date","dominant_industry"])["s"].mean()
             .reset_index().rename(columns={"dominant_industry":"industry","s":"sent_mean"}))
    sig["industry"] = sig["industry"].fillna("Unknown")
    SIG_CSV.parent.mkdir(parents=True, exist_ok=True)
    sig.to_csv(SIG_CSV, index=False)
    print(f"[OK] Built signal → {SIG_CSV}")
    return sig

def _zscore_roll(x, win=63, minp=40):
    s = x.rolling(win, min_periods=minp)
    return (x - s.mean()) / s.std(ddof=0)

def _perf_stats(r):
    r = np.array(r, dtype=float)
    r = r[np.isfinite(r)]
    if len(r) == 0:
        return {"cagr": None, "vol_ann": None, "sharpe": None,
                "max_dd": None, "hit_rate": None, "n_days": 0}
    n = len(r)
    mean_d = r.mean(); std_d = r.std(ddof=0)
    vol_ann = std_d * np.sqrt(252)
    sharpe = None if std_d == 0 else (mean_d / std_d) * np.sqrt(252)
    eq = (1 + r).cumprod()
    peak = np.maximum.accumulate(eq)
    dd = (eq / peak) - 1.0
    max_dd = float(dd.min())
    cagr = float(eq[-1]**(252.0/n) - 1.0) if n > 0 else None
    hit = float((r > 0).mean())
    return {"cagr": cagr, "vol_ann": float(vol_ann), "sharpe": None if sharpe is None else float(sharpe),
            "max_dd": max_dd, "hit_rate": hit, "n_days": int(n)}

def _backtest_variant(dfm, variant="long_only", z_thr=Z_THR, cost_bps=COST_BPS):
    # z-score of sentiment (early values are NaN)
    z = _zscore_roll(dfm["sent_mean"])
    z_vals = z.values

    # positions default to 0, then set by thresholds (NaN stays 0)
    pos = np.zeros_like(z_vals, dtype=float)
    pos[z_vals > z_thr] = 1.0
    if variant == "long_short":
        pos[z_vals < -z_thr] = -1.0

    # Avoid look-ahead: use yesterday's position on today's return
    pos_shift = np.insert(pos[:-1], 0, 0.0)
    ret = dfm["ret"].values * pos_shift

    # Trading cost at time t = |pos_t - pos_{t-1}|
    change = np.abs(np.diff(np.insert(pos, 0, 0.0)))
    cost = (cost_bps / 10000.0) * change
    cost = cost[:len(ret)]  # align

    net = ret - cost

    out = dfm[["date"]].copy()
    out["pos"] = pos_shift
    out["ret_gross"] = ret
    out["cost"] = cost
    out["ret_net"] = net
    out["eq_curve"] = (1 + out["ret_net"]).cumprod()

    stats = _perf_stats(out["ret_net"])
    stats.update({"z_thr": z_thr, "cost_bps": cost_bps, "variant": variant})
    return out, stats

def main():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    sig = _load_or_build_signal()
    if sig is None or sig.empty:
        return

    rows = []
    for ind, ticker in SECTOR_TO_ETF.items():
        edir = OUT_DIR / _slug(ind)
        edir.mkdir(parents=True, exist_ok=True)

        sec = _read_etf(ticker)
        if sec is None:
            print(f"[WARN] Missing ETF for {ind} ({ticker}).")
            continue

        s = sig[sig["industry"]==ind][["date","sent_mean"]].dropna()
        if s.empty:
            print(f"[INFO] No sentiment for {ind}.")
            continue

        dfm = sec.merge(s, on="date", how="inner").dropna().sort_values("date")
        if len(dfm) < MIN_OVERLAP:
            print(f"[INFO] Not enough overlap for backtest ({ind}). Need ≥{MIN_OVERLAP}, got {len(dfm)}.")
            continue

        # Run variants
        for variant in ["long_only", "long_short"]:
            daily, stats = _backtest_variant(dfm, variant=variant, z_thr=Z_THR, cost_bps=COST_BPS)
            daily.to_csv(edir / f"daily_{variant}.csv", index=False)
            (edir / f"summary_{variant}.json").write_text(json.dumps({"industry": ind, "ticker": ticker, **stats}, indent=2))

            # Quick equity plot
            try:
                fig = plt.figure(figsize=(6,3.2))
                plt.plot(pd.to_datetime(daily["date"]), daily["eq_curve"])
                plt.title(f"{ind} — {variant.replace('_','/').title()} (net)")
                plt.ylabel("Equity")
                plt.tight_layout()
                plt.savefig(edir / f"equity_{variant}.png", dpi=160)
                plt.close(fig)
            except Exception:
                pass

            rows.append({"industry": ind, "ticker": ticker, "variant": variant, **stats})

    if rows:
        pd.DataFrame(rows).to_csv(OUT_DIR / "summary.csv", index=False)
        print(f"[DONE] Backtest summary → {OUT_DIR/'summary.csv'}")
    else:
        print("[WARN] No backtest results written.]")

if __name__ == "__main__":
    main()
